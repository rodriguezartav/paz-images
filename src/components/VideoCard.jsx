...
export function VideoCard(props) {
  ...
  let newSrc = `video_support/${video.src.replace('videos/', '')}/thumbs_0${
    count + 1
  }.jpg`

  let videoSrc = `video_support/${video.src.replace(
    'videos/',
    ''
  )}/master_240.mp4`

  return (
    <div key={video.name} className="w-full p-2 md:w-1/2 lg:w-1/5">
      {open && <Modal video={video} setOpen={setOpen} open={open} />}
        ...
        <div>
          <div className="mb-3 flex items-center justify-between">
            <video
              ...
              src={s3Endpoint + videoSrc}
              alt=""
              controls
              preload="none"
              loop
              poster={s3Endpoint + newSrc}
            />
          </div>
          ...
          <div className="flex items-center justify-around">
            <span
              onClick={setThumb}
              className="mr-2 inline-block cursor-pointer rounded-full bg-indigo-50 py-1 px-2 text-xs text-indigo-500"
            >
              {video.type}
            </span>
...
function Modal({ open, setOpen, video }) {
  return (
